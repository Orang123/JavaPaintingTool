...
public class DrawBoard extends Function implements MouseListener,MouseMotionListener{
	...
	protected void paintComponent(Graphics g) {
		...
		if(image!=null)//如果有image就先绘制image，用于编辑图片 observer观察者是个不好的设计最好设置为null,这里也因为能够一次读取玩image
	...
	public void mousePressed(MouseEvent e) {
		...
		if(shapeChoice!=12) {//添加文本只在click监听下有用
		...
		if(shapeChoice==0||shapeChoice==2||shapeChoice==11)
	...
	public  void mouseDragged(MouseEvent e) {
		...
		else if(shapeChoice==11) {
		    index++;
		    creatAction();
		    shapesQueue[index].x1=e.getX();
		    shapesQueue[index].y1=e.getY();
		}
		else if(shapeChoice!=12){
